...
class EdgeVectorialSlow:
    ...
    def create_interface(self):
        ...
        show_edge_vectorial_aux(
            self.param, self.data, self.res, 0, curvature=False
        )
        display(out)
...
class EdgeVectorial:
    ...
    def create_interface(self):
        ...
        def mode_change(change):
            with out:
                self.set_mode(change["new"])
                plt.figure(self.fig.number)
                self.show_edge_vectorial_aux_update(
                    self.param,
                    self.data,
                    self.res,
                    time_slider.get_state()["value"],
        ...
        self.fig = plt.figure(figsize=(8, 6))
        self.show_edge_vectorial_aux_init(
            self.param, self.data, self.res, 0, curvature=False
        )
        display(out)

    def show_edge_vectorial(
        self, param, data, res, curvature=False, size=(12, 9)
    ):
        if curvature:
            name = "Edge animation with curvature"
        else:
    ...
    def show_edge_vectorial_aux_init(
        self, param, data, res, k, curvature=False
    ):
        plt.clf()
        self.p = Struct()
        self.p.t = plt.gca().set_title(
            "Frame " + str(k) + " to frame " + str(k + 1)
        )
    ...
    def show_edge_vectorial_aux_update(
        self, param, data, res, k, curvature=False
    ):
        ...
        if curvature:
            ...
            f = res.displacement[:, k]
        self.p = show_edge_scatter_update(
            param.n_curve,
            self.p,
            res.spline[k],
            res.spline[k + 1],
...
def show_edge_rasterized(param, data, res, mode=None):
    ...
    for k in range(data.K - 1):
        x = show_edge_rasterized_aux(
            param, data, res, d, dmax, k, mode, display=False
        )
...
def show_curvature(param, data, res, size=(16, 9), cmax=None, export=True):
    ...
    for k in range(data.K):
        curvature[:, k] = compute_curvature(
            res.spline[k], np.linspace(0, 1, param.n_curve, endpoint=False)
        )
...
def show_correlation_average(param, data, res, size=(16, 9), export=True):
    ...
    plt.clf()
    plt.gca().set_title(
        "Average cross-correlation between displacement and signals at layer "
        + str(0)
    )
    color = "rbgymc"
    n = 0
    for m in [0, len(data.signalfile) - 1]:
        ...
        c = correlate_arrays(res.displacement, res.mean[m, 0], "Pearson")
        plt.plot(
            t, np.mean(c, axis=0), color[n], label=data.get_channel_name(m)
        )
        n += 1
    plt.grid()
    ...
    plt.figure(figsize=size)
    plt.gca().set_title(
        "Average cross-correlation between cumulative displacement and signals at layer "
        + str(0)
    )
    color = "rbgymc"
    n = 0
