...
class TIFFSeries(VirtualData):
    def __init__(
        self, name, expdir, morphofile, signalfile, shape, K, bad_frames=[]
    ):
        self.name = name
        self.expdir = expdir
    ...
    def load_frame_morpho(self, k):
        ...
        return imread(
            self.datadir
            + self.expdir
            + self.morphofile(self.valid_frames[k] + 1)
            + ".tif"
    ...
    def load_frame_signal(self, m, k):
        return imread(
            self.datadir
            + self.expdir
            + self.signalfile[m](self.valid_frames[k] + 1)
            + ".tif"
        ).astype(dtype=np.uint16)
...
class TIFFSeriesAutodetect(VirtualData):
    ...
    def load_frame_morpho(self, t):
        return skimage.io.imread(
            os.path.join(self.cur_dir, self.segm_folder, self.segment_files[t])
        )

    def load_frame_signal(self, channel_ind, t):
        return skimage.io.imread(
            os.path.join(
                self.cur_dir, channel_ind, self.signal_files[channel_ind][t]
            )
        )
...
class MultipageTIFF(VirtualData):
    def __init__(
        self, name, expdir, morphofile, signalfile, step, bad_frames=[]
    ):
        self.name = name
        self.expdir = expdir
        self.morphofile = morphofile
