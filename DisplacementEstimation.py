...
def show_edge_image(shape, s, t, d, thickness):
	c = rasterize_curve(shape, s, 0)
	mask = -1 < c
	c[mask] = np.interp(c[mask], t, d, period=1)
	c[np.logical_not(mask)] = 0
	if thickness > 1:
		d = convolve2d(mask, np.ones((thickness, thickness)), mode='same')
		mask = 0 < d
		d[np.logical_not(mask)] = 1
		c = convolve2d(c, np.ones((thickness, thickness)), mode='same') / d
